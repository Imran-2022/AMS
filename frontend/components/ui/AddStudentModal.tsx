...
export function AddStudentModal({
  ...
  // Auto-generate student ID from backend based on class and group selection
  useEffect(() => {
    ...
    // Only generate ID when:
    // 1. Modal is open
    // 2. User has selected class AND section (not empty)
    // 3. For NEW students (no initialValues.studentId)
    // 4. Class/section/group has changed
    if (values.className && values.section && !initialValues?.studentId) {
      void generateId();
    }
  }, [values.className, values.section, values.group, classCourses, needsGroup, initialValues?.studentId]);
