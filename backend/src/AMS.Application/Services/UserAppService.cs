...
public class UserAppService : IUserAppService
{
    ...
    public async Task<UserDto> CreateAsync(CreateUserDto input, Guid currentUserId, string currentUserRole, CancellationToken cancellationToken = default)
    {
        ...
        var user = new AppUser(Guid.NewGuid(), input.FullName, input.Email, BCrypt.Net.BCrypt.HashPassword(input.Password), role, input.IsActive);
    ...
    public async Task<UserDto> UpdateAsync(Guid id, UpdateUserDto input, Guid currentUserId, string currentUserRole, CancellationToken cancellationToken = default)
    {
        ...
        if (input.FullName is not null) user = new AppUser(user.Id, input.FullName, user.Email, user.PasswordHash, user.Role, user.IsActive);
        if (input.Email is not null) user = new AppUser(user.Id, user.FullName, input.Email, user.PasswordHash, user.Role, user.IsActive);
        if (input.Role is not null && Enum.TryParse<UserRole>(input.Role, true, out var role)) user = new AppUser(user.Id, user.FullName, user.Email, user.PasswordHash, role, user.IsActive);
        if (input.Password is not null) user = new AppUser(user.Id, user.FullName, user.Email, BCrypt.Net.BCrypt.HashPassword(input.Password), user.Role, user.IsActive);
        if (input.IsActive is not null) user = new AppUser(user.Id, user.FullName, user.Email, user.PasswordHash, user.Role, input.IsActive.Value);
